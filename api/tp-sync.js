...
async function getGUServices(pool) {
...
export default async function handler(req, res) {
  ...
  const results = { files_created:0, files_updated:0, eventos_created:0, eventos_updated:0, guias_matched:0, a_designar:0, errors:[], log:[] };
  let pool = null;

  try {
    console.log('Connecting to TourPlan...');
    pool = await sql.connect(getTPConfig());
    const rows = await getGUServices(pool);
        ...
        for (const svc of booking.servicios) {
          try {
            ...
            const ed = {
              ...
              estado: guiaId?'CONFIRMADO':'A_ASIGNAR', guia_id: guiaId||null,
            ...
            if (!dryRun) {
              ...
              if (ex) {
                const upd = { tipo_evento:ed.tipo_evento, hora_inicio:ed.hora_inicio, hora_fin:ed.hora_fin,
                  pickup_location:ed.pickup_location, dropoff_location:ed.dropoff_location,
                  tp_supplier_name:ed.tp_supplier_name, tp_service_status:ed.tp_service_status, tp_synced_at:ed.tp_synced_at };
                if (ex.estado==='A_ASIGNAR' && guiaId) { upd.estado='CONFIRMADO'; upd.guia_id=guiaId; }
                await supabase.from('eventos').update(upd).eq('id',ex.id);
                results.eventos_updated++;
              } else {
                await supabase.from('eventos').insert(ed);
